...
double** pairwise_distances(POINT * points, int num_pts) {
	...
	double** dist_matrix = (double**)malloc(num_pts * sizeof(double*));
	for (int i = 0; i < num_pts; i++) {
		dist_matrix[i] = (double*)calloc(num_pts, sizeof(double));
	}
...
/*-------------------------------------------------------
	...
	INPUT:
		...
		int maxTimes: limitation for iterrations from outside if needed
	OUTPUT:
...
//Function to calculate silhouette coefficient for a single point
double calculate_silhouette_for_point(POINT* pts, int num_pts, int point_index, int k, double** distance_matrix) {
	...
	if (sum_dist_other_clusters == NULL) {
		...
		return -1.0; // Return an invalid silhouette score
	}
...
//TODO comments
// Function to calculate average silhouette score for all points
double calculate_silhouette_score(POINT* pts, int num_pts, int k, double** distance_matrix) {
	...
	for (int i = 0; i < num_pts; i++) {
		...
		sum_silhouette += calculate_silhouette_for_point(pts, num_pts, i, k, distance_matrix);
	}
...
int main()
{
	int num_pts;
	double silhouette_score = 0.0;
	//char* filename = "resources/kmeans_data.csv";
	...
	for (int i = 2; i <= num_clusters; i++) {
		...
		//printf("I have found centroids for round %d\n", i);
